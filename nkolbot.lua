...
commands.karange = function(_, range)
    ...
    local range_slider = api:get_ui_object("killaura_range")
    if range_slider then
        range_slider:SetValue(r)
    end
    send("KillAura range set to " .. r)
end

-- Shortcut for Focus Only Target
commands.focus = function(_, arg)
    local state = (arg ~= "off")
    local focus_toggle = api:get_ui_object("focus_only_target")
    if focus_toggle then
        focus_toggle:SetValue(state)
        send("Focus Only Target: " .. (state and "ON" or "OFF"))
    end
end
...
cmdBox:AddLabel("?ka → Enable Main Tab KillAura")
cmdBox:AddLabel("?karange <num> → Set KA range")
cmdBox:AddLabel("?focus on/off → Toggle focus only target")
...
local function handleChatted(msg)
    ...
    if cleanMsg == ".aura" or cleanMsg == ".ka" then
        commands.ka()
        return
    elseif cleanMsg == ".unaura" or cleanMsg == ".unka" then
        api:set_killaura(false)
        local ka_toggle = api:get_ui_object("killaura_enabled") or api:get_ui_object("enabled")
        if ka_toggle then ka_toggle:SetValue(false) end
        send("KillAura Deactivated")
        return
    end
